...
export default {
  ...
  plugins: [
    // PLAN B: composed by scripts/compose-site.mjs from
    // sites/chardonsbleus/config/plugins.yaml + plugin-registry/plugin-registry.yaml.
    // The marker below is replaced with the rendered package list at compose time.
    // Edit plugins.yaml (or `make plugin-add SITE=chardonsbleus KEY=foo`), then `make
    // compose SITE=chardonsbleus` to regenerate sites/chardonsbleus/.compiled/indiekit.config.js.
{{PLUGINS}}
  ],
